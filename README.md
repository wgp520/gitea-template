# Gitea Bootstrap 5 登陆页面主题（中文版）

本项目是根据 [gitea-template](https://github.com/sharmavijay86/gitea-template) 项目进行修改，以适应中文环境。并添加了一些优化。

- 为横幅、页脚和样式添加 .PageIsHome 条件，使其仅首页生效
- 在首页隐藏 Gitea 默认导航栏和页脚，以提供更纯净的定制体验
- 扩展自定义页脚和头部模板的显示条件，使其在登录、注册、找回密码等页面生效
- 将未登录状态下的“新建仓库”按钮替换为“登录”按钮
- 为登录相关页面添加现代化UI样式，包括渐变背景、卡片圆角、输入框和按钮美化

以下是对原项目Readme文件的翻译，并修改了展示图片。
